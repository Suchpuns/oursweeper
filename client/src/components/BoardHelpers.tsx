 ...
 */
function updateValues(board: CellAttributes[][], x: number, y: number): void {
 ...
 * 2: hard = 20, 99
 */
export function createBoard(difficulty: number): CellAttributes[][] {
  const boardSizes = [
    { x: 8, y: 10, mineNum: 10 },
  ...
  let board = [] as CellAttributes[][];
  for (let i = 0; i < boardSize.x; i++) {
    board[i] = [] as CellAttributes[];
...
function printBoard(board: CellAttributes[][]): void {
  let row: string = '';
 ...
 * @param board Minesweeper board detailing where the bombs are
 */
export function isWin(board: CellAttributes[][]): boolean {
  for (let i = 0; i < board.length; i++) {
    for (let j = 0; j < board[0].length; j++) {
 ...
 */
export function unCoverFirstTile(
  board: CellAttributes[][],
  x: number,
  y: number
): CellAttributes[][] {
  ...
  while (mineNum > 0) {
    ...
    if (board[row][col].value != -1 && !isAdjacent(row, col, x, y)) {
